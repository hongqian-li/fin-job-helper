...
**Known gap: `is_recommend_to_apply` keyword check misses paraphrased recommendations**
`agent.py`'s `is_recommend_to_apply` looks for `apply` / `recommend` / `worth applying` on the verdict's Recommendation line. A real test run against a TestCorp Cloud Architect JD scored an 8/10 match (clearly strong) but phrased the recommendation as "Consider the candidate for an interview based on their strong technical skills..." — none of the three keywords appear, so the check returned `False` and the agent skipped cover-letter talking points and the Drive save for a role that should have triggered both. Same root cause and same fix path as the "Finnish buried in a sentence" gap above: a fixed keyword list can't cover every paraphrasing the model might use, so this is left unpatched and tracked as a V5 eval test case rather than chasing individual phrasings with more keywords.
